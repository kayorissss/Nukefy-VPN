    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ...
            ACTION_START -> {
                ...
                running = true
                promoteForeground(if (mode == "tun") "TUN" else "SOCKS / HTTP")
                finish(mapOf("ok" to true, "mode" to mode))
                return START_STICKY
            }
            else -> {
                val config = File(filesDir, "run/config.json")
                if (shouldRevive() && config.exists()) {
                    val error = startCore(config.readText(), config.absolutePath, NukefyCore.hasLibbox)
                    if (error == null) {
                        running = true
                        return START_STICKY
                    }
